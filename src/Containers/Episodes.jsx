...
const Episodes = () => {
  ...
  const URILIST =
    state.searchInput?.length >= 3
      ? `${API}/episode/?name=${state.searchInput}`
      : `${API}/episode/?page=${state.page}`;

  const { isLoading, isError, data } = useQuery(
    ["episodeList", page, key],
    () => axios.get(URILIST).then((res) => res.data),
  ...
  useEffect(() => {
    if (data !== undefined && !isError) {
      ...
      dispatch({
        type: ActionTypes.SET_TOTAL_EPISODES,
